...
var port = process.env.PORT || 3004; // used to create, sign, and verify tokens
...
app.post("/login", function(req, res) {
  ...
  Application.findOne({audience:host}, function(err, application){
    if(application==null)
    {
      res.status(403);
    }
      ...
      User.findOne({
        ...
        if( user == null ){
          ...
          if(bcrypt.compareSync(req.body.password, user.password)) {
            ...
            var payload = {id: user.id, roles: user.roles, application_data:user.application_data};
